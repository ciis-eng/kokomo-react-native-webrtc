...
if [ -n "$2" ]; then
  ...
  python3 build-add-tagname.py $2
...
# Comment out for CircleCI Testing -- python3 build-webrtc.py --sync --$1 ~/srcjitsi
...
# Comment out for CircleCI Testing -- python3 build-webrtc.py --build --$1 ~/srcjitsi
